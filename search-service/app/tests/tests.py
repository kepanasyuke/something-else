...
class TestModels:
    @pytest.mark.parametrize("id_val, rubrics, text, date_str", [
    ...
    def test_document_validation(self, id_val, rubrics, text, date_str):
        doc = Document(id=id_val, rubrics=rubrics, text=text, created_date=datetime.fromisoformat(date_str))
    ...
    @pytest.mark.parametrize("query, limit, offset", [
        ("test", 10, 5), ("", 0, 0), ("hello", 30, 0), ("world", 1, 100),
        ("search", 50, 200), ("a", 1, 0), ("long query", 20, 10),
        ("политика", 15, 3), ("", 30, 0), ("x", 5, 5),
    ])
    ...
    def test_search_response(self, total):
        docs = [Document(id=i, rubrics=[], text="", created_date=datetime.now()) for i in range(total)]
...
class TestConverters:
    @pytest.mark.parametrize("row, expected_id, expected_rubrics, expected_text, expected_date", [
        ({"id": 1, "rubrics": "a,b", "text": "t1", "created_date": "2023-01-01T10:00:00"}, 1, ["a", "b"], "t1", "2023-01-01T10:00:00"),
        ({"id": 2, "rubrics": "", "text": "t2", "created_date": "2023-01-02T10:00:00"}, 2, [], "t2", "2023-01-02T10:00:00"),
        ({"id": 3, "rubrics": "x", "text": "t3", "created_date": "2023-01-03T10:00:00"}, 3, ["x"], "t3", "2023-01-03T10:00:00"),
        ({"id": 4, "rubrics": "one,two,three", "text": "long", "created_date": "2022-12-25T09:00:00"}, 4, ["one", "two", "three"], "long", "2022-12-25T09:00:00"),
        ({"id": 5, "rubrics": "['a','b']", "text": "json", "created_date": "2021-11-11T11:11:11"}, 5, ["a", "b"], "json", "2021-11-11T11:11:11"),
        ({"id": 6, "rubrics": "x,y,z", "text": "abc", "created_date": "2020-05-05T05:05:05"}, 6, ["x", "y", "z"], "abc", "2020-05-05T05:05:05"),
        ({"id": 7, "rubrics": "['single']", "text": "s", "created_date": "2019-01-01T00:00:00"}, 7, ["single"], "s", "2019-01-01T00:00:00"),
        ({"id": 8, "rubrics": "a,b,c,d,e", "text": "many", "created_date": "2023-12-31T23:59:59"}, 8, ["a", "b", "c", "d", "e"], "many", "2023-12-31T23:59:59"),
    ])
    def test_row_to_document(self, row, expected_id, expected_rubrics, expected_text, expected_date):
...
class TestDB:
    @pytest.mark.parametrize("ids, expected_result", [
        ([], []),
        ([1], [{"id": 1, "rubrics": "a", "text": "t", "created_date": "2023-01-01"}]),
        ([1, 2], [{"id": 1, "rubrics": "a", "text": "t", "created_date": "2023-01-01"},
                  {"id": 2, "rubrics": "b", "text": "t2", "created_date": "2023-01-02"}]),
        ([10, 20, 30], [{"id": 10, "rubrics": "x", "text": "y", "created_date": "2023-02-01"}]),
        ([5, 6], [{"id": 5, "rubrics": "r1", "text": "txt1", "created_date": "2023-03-01"},
                  {"id": 6, "rubrics": "r2", "text": "txt2", "created_date": "2023-03-02"}]),
        ([100, 200, 300, 400], [{"id": 100, "rubrics": "z", "text": "zz", "created_date": "2023-04-01"}]),
    ])
    @pytest.mark.asyncio
    ...
    @pytest.mark.parametrize("rowcount, expected", [(1, True), (0, False), (5, True), (3, True), (0, False)])
    @pytest.mark.asyncio
...
class TestES:
    @pytest.mark.parametrize("query, size, offset, expected_ids, expected_total", [
        ("test", 10, 0, [1, 2], 2),
        ("", 5, 0, [], 0),
        ("none", 10, 0, [], 0),
    ...
    @pytest.mark.asyncio
    async def test_search_documents(self, mock_es, query, size, offset, expected_ids, expected_total):
    ...
    @pytest.mark.parametrize("doc_id, expected", [(1, True), (2, False), (3, True), (4, False), (5, True), (10, False), (100, True)])
    @pytest.mark.asyncio
    ...
    @pytest.mark.parametrize("docs, refresh", [
        ([{"id": 1, "text": "a"}], True),
        ([], False),
...
class TestLoadData:
    @pytest.mark.parametrize("row_dict, idx, expected", [
        ({"id": "5"}, 1, 5),
        ({}, 1, 1),
        ({"id": "   "}, 2, 2),
        ({"id": "123"}, 10, 123),
    ...
    @pytest.mark.parametrize("rubrics_str, expected", [
        ("['a','b']", ["a", "b"]),
        ("[1,2]", [1, 2]),
        ("a,b,c", ["a", "b", "c"]),
        ("", []),
    ...
    @pytest.mark.parametrize("row_dict, idx, expected_id, expected_rubrics", [
        ({"id": "5", "rubrics": "['a']", "text": "t", "created_date": "2023-01-01"}, 1, 5, ["a"]),
        ({}, 1, 1, []),
        ({"id": "3", "rubrics": "x,y", "text": "txt", "created_date": "2022-01-01"}, 1, 3, ["x", "y"]),
        ({"id": "", "rubrics": "[]", "text": "", "created_date": ""}, 2, 2, []),
        ({"id": "10", "rubrics": "['alpha','beta']", "text": "greek", "created_date": "2021-01-01"}, 1, 10, ["alpha", "beta"]),
        ({"id": "7", "rubrics": "a,b,c", "text": "letters", "created_date": "2020-02-02"}, 1, 7, ["a", "b", "c"]),
        ({"id": "0", "rubrics": "[]", "text": "zero", "created_date": "2019-03-03"}, 1, 0, []),
        ({"id": "99", "rubrics": "['only']", "text": "one", "created_date": "2018-04-04"}, 1, 99, ["only"]),
    ])
    def test_build_document(self, row_dict, idx, expected_id, expected_rubrics):
        ...
        if doc is None:
            assert False
        assert doc["id"] == expected_id
        assert doc["rubrics"] == json.dumps(expected_rubrics) if expected_rubrics else '[]'


class TestMain:
    @pytest.mark.parametrize("query", [
        "", "test", "политика", "hello world", "none", "example",
        "long query with spaces", "single", "multiple words here",
        "special chars !@#", "12345", "русский язык", "   ",
        "query with numbers 123", "alpha beta gamma",
    ])
    @pytest.mark.asyncio
    ...
    async def test_search_endpoint(self, client, query, limit, offset):
        resp = await client.post("/search", json={"query": query, "limit": limit, "offset": offset})
        assert resp.status_code == 200
        data = resp.json()
        assert "results" in data
        assert "total" in data

    @pytest.mark.parametrize("doc_id", [1, 999, 1000, 0, -1, 12345, 999999, 2, 3, 4, 5, 10, 100, 200, 500])
    @pytest.mark.asyncio
    ...
    async def test_search_invalid_limit(self, client, invalid_limit):
        resp = await client.post("/search", json={"query": "test", "limit": invalid_limit})
...
class TestUI:
    ...
    @pytest.mark.parametrize("query, results, error", [
        ("test", [], ""),
        ("", None, "error"),
        ("example", [{"id": 1}], ""),
        ("hello", [{"id": 1, "text": "x"}], ""),
